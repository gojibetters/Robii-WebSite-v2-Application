...
export async function registrationUser(req, res) {
  ...
  sql.connect(config, async (err) => {
    ...
    if (!tableExists.recordset.length) {
      await request.query(
        `CREATE TABLE USERS (NAME VARCHAR(30) NOT NULL, LASTNAME VARCHAR(100) NOT NULL, EMAIL VARCHAR(100) NOT NULL PRIMARY KEY, TELEFONE VARCHAR(17) NOT NULL, PASSWORD VARCHAR(100) NOT NULL)`
      );
    }
